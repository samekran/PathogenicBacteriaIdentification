...
string job(argv[1]);
int R_all = atoi(argv[2]);      // R parameter
int B_all = atoi(argv[3]);      // B parameter
int n_perSet = atoi(argv[4]);   // n_perSet parameter
bool is_training = atoi(argv[5]); // 1 for training, 0 for testing
double p = atof(argv[6]); // p parameter
int k = ceil(-log(p) / log(2)); // Calculate k based on p

// Set flags based on training/testing
bool insert = is_training;
bool ser = is_training;
bool test = !is_training;
bool deser = !is_training;
...
// constructor
RAMBO myRambo(n_perSet, R_all, B_all, K, p, k);
...
if (insert == true){
  ...
  for (int batch =0; batch<K/100; batch++){
    ...
    cout << "Trying to open: " << dataPath << endl;  // Add this debug line
    ...
    for (unsigned int ss=0; ss<setIDs.size(); ss++){
      ...
      cout << "Trying to open: " << mainfile << endl;  // Add this debug line
